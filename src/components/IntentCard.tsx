...
export const IntentCard: React.FC<IntentCardProps> = ({
  ...
  return (
    <div className={`group bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border transition-all rounded-2xl p-5 flex flex-col justify-between shadow-sm hover:shadow-md relative overflow-hidden ${
      ...
      {/* Top Banner: Urgency & Countdown Expiration */}
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <div className={`flex items-center gap-1 text-[11px] font-mono px-2.5 py-1 rounded-lg border ${
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">
              {intent.creatorTrustScore}% Trust Score
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 pt-1">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 px-3 py-2 rounded-xl text-xs font-bold transition-all"
        >
          <Sparkles className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
          <span>Matches ({intent.matchesCount})</span>
        </button>
        ...
        {onDelete && intent.status !== 'matched' && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onDelete(intent.id);
